...
const URL = "https://port-0-likelion-hackathon-lxmynpl6f586b2fd.sel5.cloudtype.app/";
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      try {
        ...
        // 완료된 할일 개수 초기화
        const completedCount = response.data[0].user_todo.filter(todo => todo.completed).length;
    ...
    <div className={style.flex_wrapper}>
      <div className={style.steps_wrapper}>
        <nav className={style.completion_state}>
          <span>{month}월 {date}일, {week}요일</span>
          <h2>{totalTodos}개 중 {completedTodos}개 완료</h2>
        </nav>
